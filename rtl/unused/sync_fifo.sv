...
module sync_fifo #(
  ...
  always_ff @(posedge clk or posedge rst) begin
    if (rst) wr_pointer <= (ADDR_WIDTH)'(0);
    else if (sclr) wr_pointer <= (ADDR_WIDTH)'(0);
  ...
  generate
    if (LOOKAHEAD) begin
      always_ff @(posedge clk or posedge rst) begin
        if (rst) rd_pointer <= (ADDR_WIDTH)'(1);  //init to 1 for lookahead architecture
        else if (sclr) rd_pointer <= (ADDR_WIDTH)'(1);
        else if (rd_en & ~empty)  //underflow protect
          rd_pointer <= rd_pointer + (ADDR_WIDTH)'(1);
      end
    end else begin
      always_ff @(posedge clk or posedge rst) begin
        if (rst) rd_pointer <= (ADDR_WIDTH)'(0);
        else if (sclr) rd_pointer <= (ADDR_WIDTH)'(0);
        else if (rd_en & ~empty)  //underflow protect
          rd_pointer <= rd_pointer + (ADDR_WIDTH)'(1);
      end
    end
  ...
  //Word counter
  always_ff @(posedge clk or posedge rst) begin
    if (rst) word_cnt <= (ADDR_WIDTH + 1)'(0);
    else if (sclr) word_cnt <= (ADDR_WIDTH + 1)'(0);
    else if (rd_en & ~wr_en & ~empty) word_cnt <= word_cnt - (ADDR_WIDTH + 1)'(1);
    else if (wr_en & ~rd_en & ~full) word_cnt <= word_cnt + (ADDR_WIDTH + 1)'(1);
  ...
  always_ff @(posedge clk) begin
    if (ram_wren) mem[ram_wraddr] <= data_in;
  end

  //Memory Output MUX & register
  always_ff @(posedge clk) begin
    if (ram_rden) ram_q <= mem[ram_rdaddr];
  end
  ...
  generate
    if (LOOKAHEAD) begin
      ...
      //First word MUX
      always_ff @(posedge clk or posedge rst) begin
        if (rst) use_buffer <= 1'b0;
        else if (use_buffer_next) use_buffer <= 1'b1;
        else if (rd_en) use_buffer <= 1'b0;
      end

      //First word buffer
      always_ff @(posedge clk or posedge rst) begin
        if (rst) word_buffer <= (DATA_WIDTH)'(0);
        else if (use_buffer_next) word_buffer <= data_in;
      end
