...
// Coverage-dedicated top that keeps the AXI interfaces directly under
// bnn_fcc_coverage_tb so external grading scripts can attach monitors without
// traversing through an extra wrapper instance.
module bnn_fcc_coverage_tb #(
    ...
    parameter time TIMEOUT = 1s,
    parameter time CLK_PERIOD = 10ns,
    parameter bit DEBUG = 1'b0,
    parameter string DEFAULT_UVM_TESTNAME = "bnn_fcc_coverage_sweep_test",
  ...
  initial begin
    string selected_testname;

    if ($value$plusargs("UVM_TESTNAME=%s", selected_testname))
      run_test();
    else
